...
void onRequest(const HttpRequest& req, HttpResponse* resp)
{
  ...
  {
    // 根目录请求
    resp->SetStateCode(HttpResponse::HttpStateCode::K2000k);
    resp->SetStatusMessage("OK");
    resp->SetContentType("text/html");
    resp->addHeader("Server", "li");
    resp->SetBoby("<html><head><title>This is title</title></head>"
  ...
  else if(req.GetPath()=="/favicon.ico")
  {
    ...
    resp->SetBoby(string(favicon, sizeof favicon));
  }
  else if (req.GetPath() == "/hello") 
  {
    ...
    resp->SetBoby("hello, world!\n");
  }
  else 
  {
    resp->SetStateCode(HttpResponse::HttpStateCode::K404NotFound);
    resp->SetStatusMessage("Not Found");
    resp->SetCloseConnetion(true);
  }
}
